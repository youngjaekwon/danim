...
BEGIN
    WHILE(NUM1<472) --NUM이 10보다 작을때까지 LOOP실행
        LOOP
            INSERT INTO ITEMS VALUES (LPAD(ITEMS_SEQ.nextval, 6, 0), NULL, NULL, NULL, NULL, NULL, NULL, 1);
            NUM1 := NUM1+1; --NUM = NUM +1
        END LOOP;
END;

delete from ITEMS where ITEMNUM = '0000472';
...
SELECT * FROM ITEMS WHERE CATEGORY IN ('Film Camera', 'Film');
