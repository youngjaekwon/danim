...
public class ItemsDao implements IItemsDao{
    ...
    public String insert(Items item) {
        String SQL = "INSERT INTO ITEMS VALUES (LPAD(ITEMS_SEQ.nextval, 6, 0), ?, ?, ?, ?, ?, ?, ?)";
        ...
        return jdbcTemplate.queryForObject("SELECT TO_CHAR(LPAD(ITEMS_SEQ.currval, 6, 0)) FROM DUAL", String.class);
    }
    ...
    // 제품 검색(attribute를 이용한 전체 리스트 검색)
    @Override
    public List<Items> searchAllByFilters(String category, String sorting, String keyword) {
        List<Items> items = null;
        String SQL = "SELECT * FROM ITEMS WHERE CATEGORY IN " + category + " AND "
                + " (ITEMNUM LIKE ? OR MFR LIKE ? OR NAME LIKE ? OR INFO LIKE ?) " +
                "ORDER BY " + sorting;
        items = jdbcTemplate.query(SQL, new RowMapper<Items>() {
            @Override
            public Items mapRow(ResultSet resultSet, int i) throws SQLException {
    ...
    public List<Items> searchAllByFilters(String category, String stock, String sorting, String keyword) {
        List<Items> items = null;
        String SQL = "SELECT * FROM ITEMS WHERE CATEGORY LIKE ? AND (CASE WHEN TO_NUMBER(STOCK) IS NULL THEN 0 ELSE TO_NUMBER(STOCK) END) " + stock
                + " AND (ITEMNUM LIKE ? OR MFR LIKE ? OR NAME LIKE ? OR INFO LIKE ?) " +
                "ORDER BY " + sorting;
        items = jdbcTemplate.query(SQL, new RowMapper<Items>() {
            @Override
            public Items mapRow(ResultSet resultSet, int i) throws SQLException {
