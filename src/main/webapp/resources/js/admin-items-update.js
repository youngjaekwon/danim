...
function removePic(index){
  const removeTarget = document.getElementById(index);
  removeTarget.remove();
}


function updateForm(){
  const localPics = document.getElementsByClassName("localPic");
  let localPicsStrList = "";
  Array.prototype.forEach.call(localPics, (pic) => {
    localPicsStrList += pic.id + "$";
  });
  localPicsStrList = localPicsStrList.substring(0, localPicsStrList.length - 1);

  const externalPics = document.getElementsByClassName("externalPic");
  let externalPicsStrList = "";
  Array.prototype.forEach.call(externalPics, (pic) => {
    externalPicsStrList += pic.id + "$";
  });
  externalPicsStrList = externalPicsStrList.substring(0, externalPicsStrList.length - 1);

  document.updateItem.localPics.value = localPicsStrList;
  document.updateItem.externalPics.value = externalPicsStrList;
